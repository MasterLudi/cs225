...
class maze
{
  ...
  private:
	...
	/*uint64_t shortest_path_length(Graph& g, vertex start, vertex end, std::vector<direction> &dir);*/
	/*uint64_t find_path(vertex start, vertex end, bool bdir, std::vector<direction>& dir);*/
	void find_path(std::vector<direction>& dir);
	direction get_direction(vertex v, int& idx);
};
}
#endif
